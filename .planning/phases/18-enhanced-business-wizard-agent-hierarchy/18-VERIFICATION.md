...
|---|-------|--------|----------|
| 1 | Wizard includes subdomain step with availability checking and preview (subdomain.fleetfactory.ai) | VERIFIED | `wizard-subdomain-step.tsx` calls `checkSubdomainAvailability` via debounced `setTimeout(async () => { ... }, 500)`. Shows checking/available/taken badges. Preview renders `https://${subdomain}.fleetfactory.ai`. |
