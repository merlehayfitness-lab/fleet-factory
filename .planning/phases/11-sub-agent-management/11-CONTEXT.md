...
### Tree visual style
- Vertical (top-down) unified org chart — Owner agent as single root at top, departments branching down, agents and sub-agents flowing further down
- Elbow/step connecting lines between parent and children — straight lines matching classic org chart style (REVISED from bezier curves during UAT — user provided reference mockup)
- Single unified tree rooted at the Owner department's lead agent, NOT separate flat department sections
- Replaces the current agents list entirely (no toggle between list/tree — tree IS the agents page)

### Node content & density
- Consistent rectangular box nodes for ALL levels — name + role displayed, status dot (REVISED from compact pills during UAT — user wants box cards like classic org chart)
- Departments rendered as subtle uppercase labels above their lead agents, NOT as distinct header nodes (REVISED during UAT — user wants unified tree, not separate sections)
...
- All nodes should be consistent rectangular boxes — root node gets a subtle primary accent border to distinguish it
